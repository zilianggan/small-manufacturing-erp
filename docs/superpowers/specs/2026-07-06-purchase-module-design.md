...
6. **"Proceed to Purchase Order"** opens the same create/edit form dialog, prefilled from the quotation, so the vendor can adjust qty/cost before committing; submit sets `order_date=today`, `status=ORDERED`.
...
- `convertToPurchaseOrder(headerId, header, details[]): Promise<void>` — same as `updatePurchase` plus `order_date=today`, `status='ORDERED'`.
...
**Form dialog** (shared by New Quotation / Edit Quotation / Proceed to Purchase Order — title and submit label vary by mode): vendor `ComboBox`, inline material-add panel (material picker from `MaterialService.getMaterials()`, no vendor filter), line-items table (material name, qty, unit cost, total, remove), subtotal block, `AttachmentSection`. Structurally identical to today's create/edit form, just retargeted at the new service/types.
