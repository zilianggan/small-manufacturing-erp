...
- **InventoryTransaction** (see `InventoryTransactionService.ts`): `refNo?: string` — a computed/joined display field (no new DB column), populated as `purchaseHeader?.purchase_no || salesHeader?.sales_no` via the existing `purchase_detail(purchase_header(...))` / `production_material_usage(sales_detail(sales_header(...)))` joins, so an inventory transaction row can show which Sales/Purchase order it came from.
  - **Ledger search matches reference no too**: `getInventoryTransactions`'s search box claims "material, product, or reference" but for a while only resolved material/product name matches — a search by purchase/sales no silently returned zero rows. Fixed via `resolveReferenceNoIds(q)`, which mirrors the existing `resolveStatusFilterIds` shape (resolve ids through the embedded header, since the ledger row itself has no reference-no column): three parallel `ilike` lookups against `purchase_header.purchase_no`/`sales_header.sales_no` (direct sales rows and the production-material-usage route both need checking), OR'd into the same query alongside the material/product id matches.
...
Sends a Purchase/Sales quotation to the vendor/client's contact person as a pre-filled WhatsApp message — one editable template per quotation type, not a multi-template list.
- **Data**: `purchase_header.contact_id`/`sales_header.contact_id` (FK → `contacts`, optional) — the vendor-side/client-side person this quotation is addressed to, picked via a ComboBox scoped client-side to that vendor/client (not DB-enforced). `whatsapp_templates` table has exactly one row per type (`PURCHASE`/`SALES`, `type` is `UNIQUE`), edited from System Admin.
- **Rendering**: `utils/whatsapp.ts` — `buildWhatsappUrl(phone, message)` builds the `wa.me` link (phone already E.164-normalized elsewhere, wa.me just wants bare digits); `fillPurchaseTemplate`/`fillSalesTemplate` fill `{{vendor_name}}`/`{{customer_name}}`, `{{quotation_no}}`, `{{items}}` (one line per line-item) and, for Sales, `{{grand_total}}`. Purchase has no price tokens — it's a request for quotation, not an offer.
- **UI**: a WhatsApp icon button on `OrdersView`/`PurchasesView`'s list row and `SalesOrderDetailView`/`PurchaseOrderDetailView`'s detail page, wired via `onWhatsapp?: (order) => void` props, shown only when `contactPhone` is set (each view fetches the template on click via `getWhatsappTemplates`, then `window.open(buildWhatsappUrl(...))`). `SystemAdminView` gained a "WhatsApp Templates" section (`getWhatsappTemplates`/`saveWhatsappTemplate`) with a click-to-insert token editor — offers only the tokens each type's `fillPurchaseTemplate`/`fillSalesTemplate` actually substitutes.

## Browser Back Navigation & Column Visibility (v1.0.5)
- **Browser tab-history** (`App.tsx`): when running in a browser (not Electron — gated by `isBrowser = !navigator.userAgent.includes('Electron')`), switching top-level tabs pushes `{tab}` onto `window.history` and a `popstate` listener restores `activeTab` on browser Back/Forward. `useAndroidBackButton.ts` got the same `isBrowser` gate: while a drill-down detail is open it pushes a history entry so Back closes the detail (via the existing `activeHandler`) instead of leaving the page/tab; closing the detail any other way pops that entry back off. Electron's own back button/gesture doesn't exist, so none of this touches the Capacitor/Android `backButton` listener already in that file.
- **Column visibility** (`useColumnVisibility.ts` + `shell/ColumnsMenu.tsx`): per-table hidden-column set, persisted to localStorage under a `columns:<table>` key (`columns:inventory-transactions`/`columns:sales-orders`/`columns:purchase-orders`/`columns:materials`/`columns:products`). Wired on all five catalog/ledger listings — Inventory/Orders/Purchases/Material/Product — via a "Columns" popover button (`FilterBar`'s `right` slot, alongside `SortMenu` on Material/Product) that toggles which `DataTableColumn`s render (`columns.filter(c => !hidden.has(c.key))`). On Inventory/Orders/Purchases, `resetFilters` also calls `resetColumns()` so Reset clears both filters and hidden columns together; Material/Product don't have a Reset button, so their hidden-column set only clears via the menu's own "Select All".
- **Created/Modified columns** (all five listings): every listing gained sortable+hidable `createdAt`/`updatedAt` columns (Inventory gets `createdAt` only — the ledger is insert-only, no `updated_at` column exists on `inventory_transaction`). Purchase/Sales/Inventory sort server-side off a plain PostgREST `.order('created_at'|'updated_at', ...)` call (`PurchaseSortField`/`SalesSortField`/`InventoryLedgerSortField` gained the two values, no DB change needed). Material/Product route through the `get_materials_page`/`get_products_page` RPCs instead, so their sort support needed two new `WHEN 'created_at:...'`/`'updated_at:...'` branches added to each function's `v_order` CASE in `supabase/function_trigger.sql` — **this SQL edit is checked in but not yet applied**; until the user re-runs `function_trigger.sql` in the Supabase SQL editor, picking Created/Modified sort on Material/Product silently falls back to name-sort (the RPC's `ELSE` branch), even though the columns render fine either way (they're plain `SELECT`ed fields, already returned before this change).
- **Dashboard layout reset**: `DashboardView`'s customize mode gained a "Reset to Default" floating button (visible only while `customizing`) that restores both section visibility and drag order (`setPreferences(EMPTY_PREFERENCES)`, `visible_sections: {}` + no `section_order` — same as a fresh install).
...
- **Purchase/Sales import — optional inventory transaction**: the Preview screen for these two categories (only) shows an "Add inventory transaction (adjust stock)" checkbox, checked by default. It's threaded straight through as `commitPurchaseImport(groups, addInventoryTransaction)`/`commitSalesImport(groups, addInventoryTransaction)`'s second param — when unchecked, the header/detail rows still commit as RECEIVED/DELIVERED with `received_quantity`/`delivered_quantity` = full quantity (so the order reads as fully fulfilled and returns/re-receive idempotency guards stay correct), but the `inventory_transaction` insert is skipped entirely, so `material.quantity`/`product.quantity` don't move. Use case: stock was already accounted for by some other means (e.g. a prior migration) and re-importing the order history shouldn't double-count it.
