...
│   ├── InventoryListShared.tsx  # `InventoryHistoryTable` — the "Inventory List" table shared by MaterialView's and ProductView's detail panel; Order Date column shows full date+time
...
│   ├── EmployeesView.tsx     # Still old styling; email/phone are now click-to-contact links (mailto/wa.me) matching Inventory's ref-link style
