...
- **Column visibility** (`useColumnVisibility.ts` + `shell/ColumnsMenu.tsx`): per-table hidden-column set, persisted to localStorage under a `columns:<table>` key (`columns:inventory-transactions`/`columns:sales-orders`/`columns:purchase-orders`). Wired identically on Inventory/Orders/Purchases listings: a "Columns" popover button next to Reset in the `FilterBar`'s `right` slot toggles which `DataTableColumn`s render (`columns.filter(c => !hidden.has(c.key))`); each view's `resetFilters` also calls `resetColumns()` so Reset clears both filters and hidden columns together.
